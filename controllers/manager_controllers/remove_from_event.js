...
const removeFromEvent = async (req, res, next) => {
    logger.info('removeFromEvent')
        
        
        const {eventId, usherId} = req.body;

        try {
            const remaveUsher = await EventUsher.findOne({where:{
                usherId: usherId,
                eventId: eventId
            }})

            logger.error(remaveUsher)

            await EventUsher.destroy({where:{
                usherId: usherId,
                eventId: eventId
            }})
...
module.exports = removeFromEvent